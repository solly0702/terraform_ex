...
variable "file_share" {
    description = <<EOF
        capacity
            The desired size of the volume. The capacity must be a whole number followed by a size uint such as TB for terabyte. If no size unit is specified, GB is assumed. The minimum size for a standard instance is 1TB. The minimum size for a premium instance is 2.5TB.
        name
